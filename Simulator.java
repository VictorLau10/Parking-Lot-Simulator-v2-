...
public class Simulator {
	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if (r.eventOccurred(probabilityOfArrivalPerSec)) {
				Car c = r.generateRandomCar();
			...
			for (int i = 0; i < lot.getNumRows(); i++) {
				for (int j = 0; j < lot.getNumSpotsPerRow(); j++) {
					if (lot.getSpotAt(i, j) != null) {
						int timeParked = clock - lot.getSpotAt(i, j).getTimestamp();

						if (timeParked >= (8 * 60 * 60)) {
							Spot s = lot.remove(i, j);
							outgoingQueue.enqueue(s);
						} else {
							if ( r.eventOccurred(departurePDF.pdf(timeParked)) ) {
								Spot s = lot.remove(i, j);
								outgoingQueue.enqueue(s);
							}
						}
			...
			if (!incomingQueue.isEmpty() || frontCar != null) {	
				if (frontCar == null) {
					frontCar = incomingQueue.dequeue().getCar();
				}
				if (lot.attemptParking(frontCar, clock)) {
					System.out.println(frontCar + " ENTERED at timestamp " + clock + "; occupancy is at " + lot.getTotalOccupancy());
					frontCar = null;
				}
			}
			if (!outgoingQueue.isEmpty()) {
				Car c = outgoingQueue.dequeue().getCar();
				System.out.println(c + " EXITED at timestamp " + clock + "; occupancy is at " + lot.getTotalOccupancy());
			}
